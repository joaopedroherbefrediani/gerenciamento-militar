...
export default function RecrutamentoPage() {
  ...
  const [isModalSemQuestoesAberto, setIsModalSemQuestoesAberto] = useState(false)
  ...
  const gerarProva = async () => {
    if (questoesSelecionadas.length === 0) {
      setIsModalSemQuestoesAberto(true)
      return
    }
    ...
    <div className="space-y-6 pt-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          {podeVerTAF && (
            <button
              onClick={() => router.push('/dashboard/recrutamento/taf')}
              className="px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-colors font-medium flex items-center justify-center gap-2 text-sm sm:text-base"
            >
              <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isModalSemQuestoesAberto && (
        <BaseModal
          isOpen={isModalSemQuestoesAberto}
          onClose={() => setIsModalSemQuestoesAberto(false)}
          contentClassName="max-w-md"
        >
          <div className="bg-white rounded-lg shadow-xl w-full">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">Selecione as questões</h2>
              <button
                onClick={() => setIsModalSemQuestoesAberto(false)}
                className="text-gray-400 hover:text-gray-600 transition-colors"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            ...
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <p className="text-gray-700">
                    Para gerar a prova, selecione pelo menos uma questão (ou use <strong>GERAR PERGUNTAS ALEATÓRIAS</strong>).
                  </p>
                </div>
              </div>
            </div>
